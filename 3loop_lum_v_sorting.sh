...
#salvando i file con gli ID globali (che servono per le analisi lum vs V)
...
CSV_1="DATA_irradiated/before_annealing/A1_T=20_run=20260513-032137/luminosity/A1_T=20_run=20260513-032137_total.csv"

CSV_2="DATA_irradiated/annealing_T=75_h=5/A1_T=20_run=20260520-085312/luminosity/A1_T=20_annealed_75_5_run=20260520-085312_total.csv"

CSV_3="DATA_irradiated/annealing_T=75_h=25/A1_T=20_run=20260530-001924/luminosity/A1_T=20_annealed_75_25_run=20260530-001924_total.csv"

CSV_4="DATA_irradiated/annealing_T=100_h=5/A1_T=20_run=20260610-203538/luminosity/A1_T=20_annealed_100_5_run=20260610-203538_total.csv"
...
# Da calibrare in base alle fluttuazioni delle coordinate.
MATCH_RADIUS=40.0

# Cartella di output
OUTDIR="DATA_irradiated/merged_files"
...
def extract_phase(path: Path) -> str:
    ...
    Esempio:
    DATA_irradiated/before_annealing/A1_T=20_run=.../luminosity/file.csv
...
def mean_coordinates_per_local_spot(df: pd.DataFrame) -> pd.DataFrame:
    """
    Riduce le 6 righe dello stesso spot locale a una sola coordinata media.
...
for d in datasets:
    ...
    # Le coordinate e tutte le altre colonne restano identiche.
    df["spot"] = df["spot"].map(mapping).astype(int)

    # Salva il CSV con global ID.
    ...
    if "v_fin" in df_to_save.columns:
        sort_cols.append("v_fin")
...
# PRENDENDO SOLO v_fin MASSIMO
# ============================================================

# Dataset before_annealing già con global ID
before_matched = None
...
# Per ogni spot globale nel riferimento, prendo la riga con v_fin massimo.
idx_before_max = before_matched.groupby("spot")["v_fin"].idxmax()
...
for d in matched_datasets:
    ...
    # Riga con v_fin massimo per ogni spot presente in questa phase
    idx_max = df.groupby("spot")["v_fin"].idxmax()
        ...
        if gid in present_spots:
            ...
            if gid in before_templates.index:
                ...
                for other in matched_datasets:
                    ...
                    if len(possible) > 0:
                        row = possible.loc[possible["v_fin"].idxmax()].copy()
                        template_found = True
                        break
...
summary_output = outdir / "merged_max_vfin_global_ID.csv"
