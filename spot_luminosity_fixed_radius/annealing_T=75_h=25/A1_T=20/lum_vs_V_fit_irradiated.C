...
//root -l 'lum_vs_V_fit_irradiated.C("run=20260530-001924_total.csv")'
...
void lum_vs_V_fit_irradiated(const char* filename = "data.csv") {
    ...
    for (auto& entry : spots) {
        ...
        info.graph->SetTitle(Form("Spot %d: x = %.2f, y = %.2f, T = %.1f #circC - annealing 75#circC 25h;Overvoltage (V);Luminosity",
                                  spot,
                                  rows[0].x,
                                  rows[0].y,
                                  rows[0].T));
    ...
    grMax->SetTitle(Form("Luminosity at maximum overvoltage (v_{fin}=%.2f V) - annealing 75#circC 25h;Spot;Luminosity",
                         vmax));
    ...
    grMax_c51->SetTitle(Form("Luminosity at maximum overvoltage (v_{fin}=%.2f V) - annealing 75#circC 25h - focus;Spot;Luminosity",
                             vmax));
    ...
    for (auto& entry : fit_infos) {
        ...
        c7_spot->SaveAs(Form("A1_bef_ann_lum_vs_v_all_spots/A1_ann_75_25_lum_vs_v_spot%d.png",
                             spot_id));
