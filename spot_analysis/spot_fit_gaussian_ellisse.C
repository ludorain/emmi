...
// Gaussiana 2D asimmetrica + fondo costante
// par[0] = A
// par[1] = x0
// par[2] = y0
// par[3] = sigma_x
// par[4] = sigma_y
// ------------------------------------------------------------
Double_t gaus2D_ellisse(Double_t *x, Double_t *par) {
    Double_t X = x[0];
    Double_t Y = x[1];

    Double_t A  = par[0];  // ampiezza
    Double_t x0 = par[1];  // centro x
    Double_t y0 = par[2];  // centro y
    Double_t sx = par[3];  // sigma x
    Double_t sy = par[4];  // sigma y
    Double_t B  = par[5];  // fondo

    Double_t dx = (X - x0) / sx;
    Double_t dy = (Y - y0) / sy;

    return A * exp(-0.5 * (dx*dx + dy*dy)) +B;
}
...
// Funzione principale
// ------------------------------------------------------------
void spot_fit_gaussian_ellisse(const char* rootfile, const char* txtfile) {
    ...
    */
    ...
    for (int i = 0; i < (int)spots.size(); i++) {
        ...
        TF2* f2 = new TF2(fname, gaus2D_ellisse, xmin, xmax, ymin, ymax, 5);
        ...
        cout << "sigma_x = " << f2->GetParameter(3) << " +/- " << f2->GetParError(3) << endl;
        cout << "sigma_y = " << f2->GetParameter(4) << " +/- " << f2->GetParError(4) << endl;
        cout << "B      = " << f2->GetParameter(5) << " +/- " << f2->GetParError(5) << endl;
    }
