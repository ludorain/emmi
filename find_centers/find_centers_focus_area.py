...
#annealed 100° 5h
# python find_centers_focus_area.py --input "/Users/ludovicarainero/emmi/DATA_irradiated/annealing_T=100_h=5/A1_T=20_run=20260610-203538/2processed/run=20260610-203538_x=0_y=0_z=0_T=20_v=58.30_data=diff_processed.tif" --convolution  --coordinates_python "A1_annealed_100_5.txt"
# python find_centers_focus_area.py --input "/Users/ludovicarainero/emmi/DATA_irradiated/annealing_T=100_h=5/A1_T=20_run=20260610-203538/2processed/run=20260610-203538_x=0_y=0_z=0_T=20_v=58.30_data=diff_processed.tif" --convolution  --focus area 675 173 4
...
def parse_arguments():
    ...
    parser.add_argument('--focus_area', type=float, nargs=3, required=False,
                        metavar=('X', 'Y', 'RADIUS'),
...
def show_focus_crop(
    ...
    radius,
    circle_scale=1.0,
    circle_min_radius=3.0,
    title='Focused area',
    half_size=20
):
    """
    Show a cropped region of the background-subtracted image.

    The crop is centered around (xc, yc), using the same logic as --focus:
    - fixed crop half-size of 20 pixels by default;
    - coordinates clipped to image boundaries;
    - circle centered on the selected position.
    """
    display_radius = max(
        circle_min_radius,
        circle_scale * radius
    ...
    if x_min >= x_max or y_min >= y_max:
        print(f"Error: empty crop for x={xc:.2f}, y={yc:.2f}. "
              f"Check that the coordinates are inside the image.")
        return
...
if __name__ == "__main__":
    ...
    if args.focus is not None:
        ...
        show_focus_crop(
            ...
            title='Focus on defect 49 - T = 20 °C - v_over = 7 V - Annealing 75 °C 5h'
        )
    ...
    if args.focus_area is not None:
        xc, yc, radius = args.focus_area

        print(f" --- focus area: x={xc:.2f}, y={yc:.2f}, radius={radius:.2f} pix")
        ...
        show_focus_crop(
            ...
            circle_scale=args.circle_scale,
            circle_min_radius=args.circle_min_radius,
            title='Focus on defect 49 - T = 20 °C - v_over = 7 V - Annealing 100 °C 5h'

            #title=f'Focus area - x={xc:.2f}, y={yc:.2f}, radius={radius:.2f} px'
        )
