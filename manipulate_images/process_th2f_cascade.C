...
TH2F* contrast_stretching(TH2F* h, const char* newname = "h_contrast") {
    ...
    int i1  = (int)(0.002 * (values.size() - 1));
    int i99 = (int)(0.998 * (values.size() - 1));
...
// ============================================================
TH2F* remove_column_bias_median(TH2F* h, const char* newname = "h_remove_bias_median") {
std::cout << " --- remove columns bias using median" << std::endl;

    int nx = h->GetNbinsX();
    int ny = h->GetNbinsY();

    TH2F* h2 = contrast_stretching(h, "h_contrast_for_bias");
...
TH2F* remove_column_bias_fit(TH2F* h, const char* newname = "h_remove_bias_median") {
std::cout << " --- remove columns bias using fit with polinomial + gaussian" << std::endl;
 int nx = h->GetNbinsX();
    int ny = h->GetNbinsY();
...
// histname  = opzionale
// ============================================================
void process_th2f_cascade(const char* filename,
                  const char* ops = "",
                  const char* histname = "") {
    gStyle->SetOptStat(0);
