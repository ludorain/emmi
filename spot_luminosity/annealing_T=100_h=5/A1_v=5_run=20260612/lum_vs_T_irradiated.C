// ============================================================
// 
// root -l 'lum_vs_T_irradiated.C("A1_v=5_run=20260612-001017_total_global_ID.csv")'
...
void lum_vs_T_irradiated(const char* filename = "data.csv", int selected_spot = 3) {
    ...
    gr_c1->SetTitle(
        Form("Luminosity at maximum temperature T = %.1f #circC, v = %.1f After annealing 100 #circC 5h;Spot;Luminosity",
             Tmax, v_const)
    );
    ...
    gr_c2->SetTitle(
        Form("Luminosity at maximum temperature T = %.1f #circC, v = %.1f After annealing 100 #circC 5h - focus;Spot;Luminosity",
             Tmax, v_const)
    );
    ...
    c3->SaveAs("A1_ann_100_5_lum_vs_T_first8.png");
    ...
    if (spots.find(selected_spot) != spots.end()) {
        ...
        gr->SetTitle(
            Form("Spot %d: x = %.2f, y = %.2f, v = %.1f;T (#circC) After annealing 100 #circC 5h;Luminosity",
    ...
    gr_B->SetTitle("Exponential fit parameter B vs spot ID after annealing 100 #circC 5h;Spot;B");
    ...
    TCanvas* c6 = new TCanvas(
        ...
        "Chi square exponential fit vs spot ID after annealing 100 #circC 5h",
        1000,
        800
    );
    ...
    gr_chi2->SetTitle("#chi^{2} of exponential fit vs spot ID after annealing 100 #circC 5h;Spot;#chi^{2}");
