//root -l 'lum_vs_V_fit_irradiated.C("A1_T=20_annealed_100_5_run=20260610-203538_total_global_ID.csv")'
...
void lum_vs_V_fit_irradiated(const char* filename = "data.csv") {
    ...
    grMax->SetTitle(
        Form("Luminosity at maximum overvoltage (v_{fin}=%.2f V) annealing 100#circC 5h;Spot;Luminosity",
            vmax));
    ...
    c5->SaveAs("A1_ann_100_5_lum_vs_v_max_vfin.png");
    ...
    grMax_c51->SetTitle(
        Form("Luminosity at maximum overvoltage (v_{fin}=%.2f V) annealing 100#circC 5h - focus;Spot;Luminosity",
            vmax_c51));
    ...
    c51->SaveAs("A1_ann_100_5_lum_vs_v_max_vfin_focus.png");
    ...
    c6->SaveAs("A1_ann_100_5_lum_vs_v.png");

    // ============================================================
    // CANVAS 7: luminosity vs V_over for a single spot with fit
    // ============================================================
    TCanvas* c7 = new TCanvas("c7", "Spot 3 - Luminosity vs overvoltage", 800, 600);

    // prendi solo spot 2
    int spot_3 = 3;
    vector<Row> rows_3 = spots[spot_3];

    // ordina per overvoltage
    sort(rows_3.begin(), rows_3.end(),
        [](const Row& a, const Row& b) {
            return a.v_fin < b.v_fin;
        });
    ...
    vector<double> x_3(n), y_3(n), ex_3(n), ey_3(n);

    for (int i = 0; i < n; i++) {
        x_3[i]  = rows_3[i].v_fin;
        y_3[i]  = rows_3[i].luminosity;
        ex_3[i] = 0.0;
        ey_3[i] = rows_3[i].error;
    }

    TGraphErrors* gr = new TGraphErrors(n, x_3.data(), y_3.data(), ex_3.data(), ey_3.data());

    gr->SetTitle(Form("Spot %d: x = %.2f, y = %.2f, T = %.1f #circC annealing 100#circC 5h",
                    spot_3, rows_3[0].x, rows_3[0].y, rows_3[0].T));
    ...
    // fit
    TF1* f_quad = new TF1("f_quad_3", "[0] * TMath::Power(x,[1])", 0, 10);
    f_quad->SetParameters(1.0, 2.0);
    gr->Fit(f_quad, "Q");


    gr->Draw("AP");
    c7->SaveAs("A1_ann_100_5_lum_vs_v_spot3.png");
...
*/
    ...
    grB->SetTitle("Power law exponent:B annealing 100#circC 5h ;Spot;B");
    grB->SetMarkerStyle(21);
    grB->Draw("AP");
    grB->GetXaxis()->SetLimits(-1, nspots + 0.5);
    ...
    leg9->Draw();
    c9->SaveAs("A1_ann_100_5_lum_vs_v_B.png");
    ...
    //c10->SaveAs("A1_ann_100_5_lum_vs_v_chi2.png");
}