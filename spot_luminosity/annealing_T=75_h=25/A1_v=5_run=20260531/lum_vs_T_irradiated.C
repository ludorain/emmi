// ============================================================
// 
// root -l 'lum_vs_T_irradiated.C("A1_v=5_annealed_75_25_run=20260531-095134_total_global_ID.csv")'
...
void lum_vs_T_irradiated(const char* filename = "data.csv", int selected_spot = 3) {
    ...
    gr_c1->SetTitle(
        Form("Luminosity at maximum temperature T = %.1f #circC, v = %.1f - Annealing 75#circC 25h;Spot;Luminosity",
             Tmax, v_const)
    );
    ...
    gr_c2->SetTitle(
        Form("Luminosity at maximum temperature T = %.1f #circC, v = %.1f Annealing 75#circC 25h - focus;Spot;Luminosity",
             Tmax, v_const)
    );
    ...
    c3->SaveAs("A1_ann_75_25_lum_vs_T_first8_expfit.png");
    ...
    if (spots.find(selected_spot) != spots.end()) {
        ...
        gr->SetTitle(
            Form("Spot %d: x = %.2f, y = %.2f, v = %.1f;T (#circC) Annealing 75#circC 25h;Luminosity",
    ...
    gr_B->SetTitle("Exponential fit parameter B vs spot ID annealing 75#circC 25h (#chi^{2}/ndf < 2);Spot;B");
    ...
    TCanvas* c6 = new TCanvas(
        ...
        "Chi square exponential fit vs spot ID Annealing 75#circC 25h",
        1000,
        800
    );
    ...
    gr_chi2->SetTitle("#chi^{2} of exponential fit vs spot ID Annealing 75#circC 25h;Spot;#chi^{2}");
