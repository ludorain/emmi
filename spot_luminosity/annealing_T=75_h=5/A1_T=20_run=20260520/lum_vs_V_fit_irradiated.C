...
void lum_vs_V_fit_irradiated(const char* filename = "data.csv") {
    ...
    grMax->SetTitle(
        Form("Luminosity at maximum overvoltage (v_{fin}=%.2f V) annealing 75#circC 5h;Spot;Luminosity",
            vmax));
    ...
    grMax_c51->SetTitle(
        Form("Luminosity at maximum overvoltage (v_{fin}=%.2f V) annealing 75#circC 5h - focus;Spot;Luminosity",
            vmax_c51));
    ...
    gr->SetTitle(Form("Spot %d: x = %.2f, y = %.2f, T = %.1f #circC annealing 75#circC 5h",
    ...
    gr->Draw("AP");
    c7->SaveAs("A1_ann_75_5_lum_vs_v_spot3.png");
