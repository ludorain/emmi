...
void lum_vs_T_two_v(const char* filename = "data.csv") {
    ...
    for (const auto& [spot, points] : data) {
       ...
       get_local_range(points, 7.0, xmax_local, ymax_local);
        ...
        gPad->DrawFrame(10, 0, xmax_local, ymax_local,Form("Spot %d;T;luminosity", spot));
...
*/
    ...
    // ------------------------------------------------------------
    // Canvas 34: due pad (v = 7 e v = 9) + legenda unica
    // ------------------------------------------------------------
    TCanvas* c34 = new TCanvas("c34", "v = 7 vs v = 9", 1200, 600);
    c34->Divide(2,1);

    // Creo UNA sola legenda
    TLegend* leg = new TLegend(0.75, 0.15, 0.95, 0.90);
    ...
    c34->cd(1);
    gPad->SetLogy();
    gPad->DrawFrame(15., 6, 23, ymax,
                    "All spots - v = 7.0;T;luminosity");

    for (const auto& [spot, points] : data) {
        if (spot == 8) continue; // escludo spot 8
        if (spot == 5) continue; // escludo spot 5
        int color = colors[spot % colors.size()];
        TGraphErrors* gr = make_graph(points, 7.0, color);
        gr->Draw("PL SAME");

        // aggiungo alla legenda SOLO qui (una volta sola)
    ...
    latexL.DrawLatex(0.15, 0.85, "v = 7.0");*/
    ...
    c34->cd(2);
    gPad->SetLogy();
    gPad->DrawFrame(15.5, 6, 23.5, ymax,
                    "All spots - v = 9.0;T;luminosity");

    for (const auto& [spot, points] : data) {

        int color = colors[spot % colors.size()];  // stessi colori
        TGraphErrors* gr = make_graph(points, 9.0, color);
        gr->Draw("PL SAME");
    }
    leg->Draw();
    ...
    latexR.DrawLatex(0.15, 0.85, "v = 9.0");*/
    ...
    for (const auto& [spot, points] : data) {
        ...
        const Point& p0 = points[0];
        gPad->DrawFrame(15, 0, 23, ymax_local, Form("Spot %d: x = %.2f, y = %.2f ; T ;luminosity", spot, p0.x, p0.y));
     ...
     // ------------------------------------------------------------
    // Canvas 51: spot selezionato, v = 7.0 e v = 9.0 insieme
    // ------------------------------------------------------------
     TCanvas* c51 = new TCanvas("c51_v7_v9", "Luminosity vs T - v = 7.0 and v = 9.0", 1400, 900);
    ...
    leg51->AddEntry(gr7, "V_over = 7.0", "pl");
    leg51->AddEntry(gr9, "V_over = 9.0", "pl");
    leg51->Draw();
    ...
    TCanvas* c6 = new TCanvas("c6_ratio_T16_T22", "Ratio  L(T=22)/L(T=16) vs spot", 1000, 800);
    ...
    gPad->DrawFrame(-1, 0.2, nspots, ymax_ratio,
                    "Ratio L(T=22#circ C) / L(T=16#circC);spot;L_{22}/L_{16}");
    ...
    leg6->AddEntry(gr_ratio_v7, "V_over = 7.0", "p");
    leg6->AddEntry(gr_ratio_v9, "V_over = 9.0", "p");
    ...
    leg6->AddEntry(fit_v7, Form("fit const V_over=7: #chi^{2}/ndf = %.2f/%d", chi2_v7, ndf_v7), "l");
    leg6->AddEntry(fit_v9, Form("fit const V_over=9: #chi^{2}/ndf = %.2f/%d", chi2_v9, ndf_v9), "l");
    leg6->Draw();
