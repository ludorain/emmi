...
void lum_vs_V(const char* filename = "data.csv") {
   ...
   // ============================================================
    // CANVAS 1.1: luminosity vs v per un singolo spot
    // ============================================================
TCanvas* c11 = new TCanvas("c11", "Spot 3", 800, 600);

for (auto &entry : spots) {

    int spot = entry.first;

    if (spot != 3) continue;  // <-- filtra solo spot 3
    ...
    gr->SetTitle(Form("Spot %d: x = %.2f, y = %.2f, T = %.1f #circC",
                      spot, rows[0].x, rows[0].y, rows[0].T));
    ...
    gr_ratio->SetTitle("Luminosity variation: L(V_over=8V) / L(V_over=2V)");
    gr_ratio->GetXaxis()->SetTitle("Spot");
    gr_ratio->GetYaxis()->SetTitle("L(8V) / L(2V)");

    gr_ratio->SetMarkerStyle(20);
    gr_ratio->SetMarkerSize(1.2);
    gr_ratio->SetLineWidth(2);
    gr_ratio->GetXaxis()->SetLimits(-1, nspots - 0.5);
    gr_ratio->Draw("AP");
    ...
    leg2->AddEntry(fit, "Constant fit", "l");
